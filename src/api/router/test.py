...
async def smoke_test() -> dict[str, Any]:
    """Дымовое тестирование модели (быстрая проверка базовой функциональности)."""
    try:
        ...
        logger.info(f"Дымовой тест пройден. Точность: {scores.get('accuracy', 0):.4f}")
        ...
        logger.error(f"Дымовой тест не удался: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Дымовой тест не удался: {str(e)}")
